...
# --- Configuration ---
DATA_DIR = config.GECKO_DAILY_DATA_DIR
TOP_N_COINS = 1200
...
def get_ohlcv_history(coin_id, days='365'):
    ...
    try:
        ...
        return df[['date', 'coin_id', 'close', 'volume', 'log_Return']]
...
def process_single_coin(coin_id):
    ...
    try:
        if os.path.exists(file_path):
            # --- UPDATE LOGIC ---
            update_df = get_ohlcv_history(coin_id, days=UPDATE_LOOKBACK_DAYS)
            ...
            df = get_ohlcv_history(coin_id, days='365')
